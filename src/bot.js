...
} from "../Commands/Handlers/VoiceHandlers.js";
import {
    morningHandler
} from "../Commands/Handlers/DayNightHandlers.js";
import {
    checkVictory
} from "../Commands/Handlers/VictoryHandlers.js";
import {
    stopAllGames
        ...
        if (interaction.isButton()) {
            ...
            if (currentGame) {
                if (await checkIfDead(interaction.user.id, currentGame.id)) {
                    await interaction.reply({
                        content: `You are dead. You can't vote.`,
                        ephemeral: true
                    });
                } else {
                    ...
                    if (interaction.customId.startsWith('mafia_vote_')) {
                        try {
                            ...
                            await interaction.reply({
                                content: `Mafia ${interaction.user.username} has voted for <@${userId}>!`,
                                ephemeral: false
                            });
                    ...
                    if (interaction.customId.startsWith('doctor_vote_')) {
                        try {
                            ...
                            console.log(`User ID: ${interaction.user.id} voted for ${userId}.`);
                            await interaction.reply({
                                content: `You have voted for <@${userId}>!`,
                                ephemeral: false
                            });
                    ...
                    if (interaction.customId.startsWith('detective_vote_')) {
                        try {
                            const userId = interaction.customId.split('_')[2];
                            await interaction.reply({
                                content: `You have voted for checking the <@${userId}>\'s role! You will receive a message in the morning with results of your check. `,
                                ephemeral: false
                            });
                        ...
                        try {
                            if (currentGame) {
                                ...
                                if (userRole === 'dead') {
                                    ...
                                    interaction.reply({
                                        content: `You have voted for <@${userId}>! If you want to change your mind just click on somebody's else button.`,
                                        ephemeral: true
                                    }).then(message => {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (detectiveActionResult) {
                        ...
                        await detectiveChannel.send({
                            embeds: [detectiveActionEmbed]
                        });
                    }
                } catch (error) {
                    console.error('Error processing night actions:', error);
